...
**Applied 2026-06-24:** all [EDIT] verdicts landed — `the-12-stop-ramp.md`,
`for-designers.md`, `collisions.md`, `neutrals.md` corrected to the Stage-2 role
model + live signal-shift mechanism. [REWRITE] README done earlier (29934bc).
[KEEP] files unchanged. Token names verified against `tokenNames.ts`; live
collision/shift facts verified against `signalShift.ts` / `resolve.ts` /
`collision.ts` (dead `signalYieldShift`/`warningVariant` confirmed callerless).

**Stage 4 — signal identity rename (2026-06-24):** the engine's signal keys
`error/warning/success/info` were renamed to identity `red/yellow/green/info-color`
