...
- `subtle`: the family's `highlighter-26` at an opacity rung, resting at `008` and climbing one rung per state: `subtle-bg-enabled` 008, `subtle-bg-hover` 012, `subtle-bg-pressed` 016, `subtle-bg-selected` 024.
...
AA body text holds on every rung: `fg` and `fg-strong` clear 4.5:1 over every `subtle` and `hint` row composited over every paper of the family and of the neutral, the inverse family over the inverted grounds, in both modes. `audit:tokens` holds this across the fixture roster.
