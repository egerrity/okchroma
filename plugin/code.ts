...
// modes' rename-proofing: display names are the user's to change, the stored
// ids are the contract (the collections' tag idiom).
const MODE_IDS_KEY = 'okchroma-mode-ids'
// Each apply stamps its input recipe here (the ext SPEC_KEY idiom): ONE JSON map on
// the THEME collection, keyed by lower-cased brand — what powers "Edit applied theme"
// and "Re-apply all brands" without retyping. A brand applied before recipes existed
// is RECONSTRUCTED from the file in collect-specs.
const SPEC_KEY = 'okchroma-spec'
// Variable identity: the canonical path lives in plugin data — the
...
// Generation stamp. The instruments rename's inverted digits made two pre-Stage-B
// INDEX spellings live names again: paper-1 (the same stop in both vintages) and
// paper-3 (index 3 = the 0.95 stop in the index vintage; the 0.97 stop here). A row
// is ours only if it carries the current generation — see getOrMigrate.
const GEN_KEY = 'okchroma-gen'
...
// Posture stamp, written only into APCA-pair files (their solve lane can't be described
// with WCAG conformance phrases). WCAG files get per-variable descriptions from
// tokenDescriptions.ts — a shared stamp's ratio digits would pollute Figma's picker
...
// rename is one more entry). Mirrored in plugin-ext/code.ts.
// The pen renumber entries (C14) shift every name DOWN by one; they are safe only
// because tokens are processed in ladder (ascending) order and each migration
...
const RENAMED_LEAVES: Array<[string, string]> = [
  ...
  // the NAMES (descriptions carry the conformance lines, so an "aaa" search still
  // lands); brand/secondary → brand/alt rides RENAMED_GROUPS,
  ...
  // belongs to surface/low via the entry further down — resolution ORDER
  // disambiguates, see the plane-order note there.
  ...
  // below points STRAIGHT at the final flat home — the one-hop rule). A C14-vintage
  // file holds BOTH ink-11 (scale, hyphen flat) and ink-12 (anchor, hyphen flat) —
  // resolving pen-70 must consume ink-11 before the pre-renumber ['ink-12','pen-70']
  // entry can capture the anchor. Ascending processing + self-deleting consumed keys
  // keep the chains sound, exactly as the renumber entries.
  ['paper-2', 'paper-3'],
  ['paper-3', 'paper-5'],
  ['wash-4', 'chalk-8'],
  ['wash-5', 'chalk-11'],
  ...
  // highlight-9 / on-highlight are DEAD (the C33 collapse orphaned them; no live
  // token answers to either) — kept only so an old file's row is still FOUND
  // (renamed to its retired banded home, then reported as an orphan) rather than
  // silently left under its flat name. No Stage B target: retired before Stage B,
  // no leaf of theirs survives to relabel.
  ['highlight-9', 'highlight/9'],
  // ── pen flats under C49 numbering (mirrors plugin-ext; the C14-vintage flat names
  // map number-true because C49 gives the strong pen and anchor their pre-C33
  // numbers back). Targets point straight at the final FLAT homes.
  ...
  // strong-pen row to the new stop. Community is unsupported; port the textUpshifts
  // idiom from plugin-ext/code.ts before re-listing.
  ['ink-9', 'pencil-47'],
  ['ink-10', 'pencil-47'],
  ['ink-11', 'pen-70'],
  // (['ink-12','pen-100'] lives below, at the anchor's flat-home note)
  // ── STAGE-B BANDED ENTRIES for spellings the flattening batch at the top does not
  // already cover (the pre-Stage-B banded digit vintage, between the band nesting and
  // Stage B). Ordering: a banded-digit file holds both ink/9 (first text) and ink/10
  // (the between stop) as two DIFFERENT real variables; resolving
  // pencil-47 (stop 9) must consume this batch's ink/9 BEFORE the collapse-era
  ...
  // onto the pen stops and keeps resolving.
  ...
  // one-hop rule — legacyCandidates never chains). The pen renumber entries
  ...
  ['ink-12', 'pen-70'],
  ['ink-13', 'pen-100'],
  // blue-signal variant relabels (C17, info-color → blue): the variant leaf is
  ...
  ['wash-3', 'paper-5'],
  // the anchor's flat home 'pen-100': the family anchor's banded ink/0 home is covered
  // by the flattening batch at the top; its ink-13/ink-12 flat vintages ride the
  // ['ink-13','pen-100'] entry above and ['ink-12','pen-100'] here. (No system-root
  // lane: the resolved anchor rides the neutral tint prim, so an old file's static
  // system/pen-100 row orphans unwritten.)
  ['ink-12', 'pen-100'],
  ...
  // the raised plane; the two darkest planes are edge cases and read as such.
  // ⚠️ 'base' SURVIVES THE RENAME BUT MOVES PLANES (sink|base|lift|pop's lift —
  // cards/menus — is sunken|low|base|high's base), so ORDER is load-bearing twice
  // over: surface/low must be RESOLVED before surface/mid wherever these rows are
  // ensured (STATIC_UTILS + SYSTEM_GLOBALS list order below) — an exact-name hit on a
  // file's old base row would otherwise hand the PAGE plane's variable (and its
  // bindings) to the cards name before the low-lookup could consume it.
...
// re-pointable in-between tier, so identity names would lie under a future
// info-from-secondary option. The PRIMITIVE lanes keep identity paths
// (system/red/… — machinery, hidden), so the system/info-color entry (C17)
...
const RENAMED_GROUPS: Array<[string, string]> = [
  // the family rename: the theme-layer role group brand/secondary → brand/alt
...
// Invisible-rename normalization (ext-plugin mirror; the defect: a group renamed and
// restored with a trailing space leaves every unstamped row unfindable). A
...
// mirror: a display name spelling a RETIRED engine path is a
...
function getOrMigrate(map: Map<string, figma.Variable>, path: string): figma.Variable | undefined {
  ...
  // index-era row (index stop 3 = the live paper-5), not ours: fall through to the legacy
  ...
  if (v && !indexEraCollision) {
    ...
    v.setPluginData(PATH_KEY, path); v.setPluginData(GEN_KEY, GEN_CURRENT)
    return v
  }
...
// of insertion order (ECMA-262 OrdinaryOwnPropertyKeys). Every ramp leaf is
// band-word-prefixed (paper-1, chalk-8, …) so no group consists of bare-digit keys —
// but the walker keeps the rule so the panel order can't silently reverse if a
// digit-keyed group ever returns (the defect bare-digit paper/chalk leaves had).
...
// at one brand and succeeds on manual re-run). Applies are idempotent, so the recovery
...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'apply') {
    ...
    const applyOnce = async () => {
      ...
      // posture detection reads through the rename tables — an old file may spell
      ...
      const fileHasSecondary = hasThemePath('brand/alt/paper-1')
      ...
      // Resolved RENAME-PROOF: stored modeIds first (Figma keeps a modeId across
      // renames), then the canonical names, then POSITION (first adoption of an
      // unstamped file). The plugin only NAMES modes it creates; a user rename
      // ("light"/"dark", "day"/"night") is respected — a force-rename every apply
      // would revert it.
      const p = resolveOwned(collections, MODE_NAME, profile, suffixed)
      const stamp = profileStamp(profile)
      // Per-variable descriptions in the WCAG lane (the digit-free search fix); an
      // APCA pair keeps the posture stamp — WCAG conformance phrases would lie there.
      const descFor = (path: string) => (profile === 'apca' ? stamp : describeToken(path))
      // Web code syntax = the hyphenated display name — raw kebab, no var(--…)
      // wrapper; restamped every apply, so it follows a user's panel rename
      ...
      // (with Light hand-deleted, the positional fallback would otherwise claim the
      // dark survivor and write light values into it)
      ...
      // the family rename: brand/<name>/secondary/ → brand/<name>/alt/.
      ...
      //   - NEITHER anchor is static: the engine resolves both and they ride the
      //     neutral ramp at system/neutral/<tint>/paper-0 and .../pen-100 (near-black
      //     light / near-white dark, never the literal pole). A file's static
      //     system/pen-100 row from the static-anchor vintage orphans in place
      //     (scopes [], unbindable — the red-variant class); its theme alias
      //     re-points on re-apply.
      const W = { r: 1, g: 1, b: 1 }
      const K = { r: 0, g: 0, b: 0 }
      // The list order IS the display order in Figma (variables list in creation
      // order; the owner's panel layout: abs poles at the system root, then surface/,
      // then alpha/). The four elevation planes surface/dim|low|mid|high are
      // mode-divergent aliases set later — once the theme's neutral papers exist.
      ...
      // climbs paper-5→paper-0 in light, paper-0→paper-5 in dark, so a number
      // would lie.) ⚠️ ORDER: surface/low BEFORE surface/mid — the word base names
      // the raised plane in one vintage and the page plane in an older one, and
      // getOrMigrate must let the low-lookup consume a file's old base row first (see
      // RENAMED_LEAVES). The alpha/shadow ladder is pure black at 4/8/12% light; dark
      // is heavier by necessity — near black a light-mode alpha vanishes — at
      // 32/48/64%.
      ...
      for (const u of STATIC_UTILS) {
        // getOrMigrate (not .get): renamed rows (the surface-plane words, older
        // vintages) are adopted in place instead of orphaned.
      ...
      // An on-fill is a pure pole — or (C43) the default-model secondary's soft
      // on-cta: the pole at partial alpha, which aliases system/alpha/ink instead.
      // Solid poles alias PER MODE to abs-white/abs-black (mode-divergent alias, like
      // the elevation pair) instead of duplicating a value. Decoupled from the
      // paper-0/pen-100 anchors on purpose: BOTH anchors are RESOLVED colors (tinted,
      // never the pole) — on-CTA text must stay a true pole, so it aliases the abs
      // rows, never an anchor.
      ...
      // (per-brand ramps).
      // (No text-cta branch: the payload carries no text-cta reference leaves — the
      // text register IS the pen stops.)
      const writeRaw = (
        path: string,
        ...
        const created = !v
        if (!v) { v = figma.variables.createVariable(path, p.coll, 'COLOR'); v.setPluginData(PATH_KEY, path); v.setPluginData(GEN_KEY, GEN_CURRENT); primByName.set(path, v) }
        ...
        if (t.path === STAMP_LEAF.ON) {
          ...
          // soft leaf would alias the abs pole and silently DROP its alpha.
      ...
      // Every shared prim REFRESHES, never create-once. The LINK prim is seed-keyed, so
      // the path survives an engine retune while its six values move — a stale reuse
      // would serve old hover/pressed/dark values under every theme alias. Same seed ⇒
      // same engine output, so the refresh is idempotent across brands sharing the
      // prim. The SIGNAL prims follow the same rule (a create-once base strands a
      // canonical move such as C42/C44 — a stale warning pencil-47 failing on papers):
      // their identity/variant paths survive engine retunes exactly as link's does, and
      // the same idempotence argument holds. The NEUTRAL too (a create-once
      // system/neutral/default-h* would serve a retired tint's values forever): its
      // values derive entirely from the level+hue in its key, so the refresh is
      ...
      // NEUTRAL CTA ESCAPE (C20): the escaped cta REST anchors at the brand-neutral's
      // pen-70 (strong) by construction, so the primitive ALIASES the neutral's own
      // strong pen (the on-cta→pen idiom — the relationship stays live in Figma; one
      ...
      // GUARD: alias only when the target's live values equal the freshly written
      // escape cta (both modes); otherwise the raw write already carries the correct
      // values. The neutral refreshes on every apply, so a stale strong pen cannot
      // arise — the value check is the cheap invariant that keeps the alias honest.
      if (ctaEscape) {
        ...
        // the whole escape family aliases the neutral's own registers ("cta AND cta
        // pen"): fill rest → neutral pen-70 (strong); the brand's PEN STOPS →
      ...
      // An existing collection keeps its creation order — no reorder API; drag or re-apply
      // on a fresh file.

      // migration: a file from the system-anchor vintage holds the anchor under theme
      // system/ink-13 — RENAME moves it into the neutral group with every user binding
      // intact. It parks on the vintage name neutral/ink-13, NOT the final pen-100:
      // parking keeps the ascending RENAMED_LEAVES discipline — the ladder consumes the
      // scale's ink-11/ink-12 rows first, then its ['ink-13','pen-100'] entry finishes
      // the anchor's move, the same path a file already on neutral/ink-13 takes via
      // aliasInto's getOrMigrate. Jumping a migration past the ladder is how a
      // same-name collision hijacks the anchor into a scale row (mode-flipping
      // #000/#fff bindings turn brand-tinted) and orphans the real scale variable.
      // (Guard: never clobber an existing neutral/ink-13.)
      ...
      if (staleInk && !themeByName.has('neutral/ink-13')) {
        ...
        staleInk.setPluginData(PATH_KEY, 'neutral/ink-13'); staleInk.setPluginData(GEN_KEY, GEN_CURRENT)
      ...
      // ② neutral. pen-100 the anchor is IN the payload group (engine-resolved off the
      // pole — the paper-0 posture), already in ladder position after pen-70, so the
      // generic walk aliases it onto the tint prim like every other leaf. A file's
      // existing neutral/pen-100 theme var is the same var — getOrMigrate finds it and
      // this brand's mode re-points from a static-vintage pole to the tint prim.
      ...
      // ③b the system ABSOLUTES: identity sits with the poles, outside the family
      // groups — system/abs-primary / abs-alt
      ...
      // Backfill runs EVERY apply (gating on first appearance would leave a migrated
      // file's inherited create-default black modes standing):
      ...
      // ⑤ the SYSTEM LINK trio (C20: "link is a system level color… a primitive that
      // internally aliases the primary pen 10 unless it's being deconflicted"). ONE
      // trio per theme, per brand mode: DEFAULT aliases this brand's pen stops directly
      // (the neutral escape re-points the link automatically because the escape swaps
      ...
      // BACKFILL on first appearance: freshly created theme vars hold the
      // create-default (black) in every OTHER brand mode — the
      // backfillSecondary idiom. Each pre-existing mode gets its own brand's DEFAULT
      // posture (its pen stops; the custom seed belongs to the applying brand only,
      // matching the ext model where each extension overrides with its own). findPrim
      // reads through the rename tables, so any vintage's pen rows are found; the
      // pen-58 fallback covers a leaf hand-deleted from an old file (better a
      // static link than black). Later brand applies set their own mode and win over
      // this.
      if (linkIsNew) {
        // resolve prims READ-ONLY through the rename tables: an untouched brand's prims
        // may still carry older leaf names (in-place migration only runs for families
        // the CURRENT apply writes) — a raw lookup would miss them and silently leave
        // the create-default black in that mode. getOrMigrate is avoided on purpose:
        // renaming another brand's prim here would be a write side-effect on a family
        // this apply doesn't own.
        ...
        for (const m of th.coll.modes) {
          ...
          for (const [themeLeaf, brandLeaf] of LINK_LEAVES) {
            ...
            const cur = themeVar?.valuesByMode[m.modeId]
            if (cur && typeof cur === 'object' && 'type' in cur) continue
            const target = findPrim(primary + brandLeaf) ?? findPrim(primary + 'pen-58')
            if (target) aliasInto(`system/${themeLeaf}`, target, m.modeId)
          }
        }
      }

      // ⑤bis the INVERSE link trio: the link seed re-solved for text on inverted
      // surfaces. NO default-alias posture exists — no brand row carries
      ...
      // Elevation planes — mode-DIVERGENT aliases: both themes use the SAME four
      // neutral stops in REVERSED order, so each plane points at a different stop per
      // mode. Every end is the brand-aware THEME neutral (paper-0 is the engine's
      // RESOLVED anchor — white in light, one seam below paper-1 in dark, never
      // absolute black):
      //   dim  → neutral/paper-5 in light · neutral/paper-0 (deep) in dark
      //   low  → neutral/paper-3 in light · neutral/paper-1 in dark
      //   mid  → neutral/paper-1 in light · neutral/paper-3 in dark
      //   high → neutral/paper-0 (white) in light · neutral/paper-5 in dark
      // The vars are CREATED in order above; aliases are set HERE because the theme's
      // neutral vars only exist after the alias loop ("the wait"). This mirrors the
      // CSS semantic layer's surface-dim/low/mid/high exactly.
      const themeNeutralP0 = themeByName.get('neutral/paper-0')
      const themeNeutralP1 = themeByName.get('neutral/paper-1')
      const themeNeutralP2 = themeByName.get('neutral/paper-3')
      const themeNeutralP3 = themeByName.get('neutral/paper-5')
    ...
    // "Edit applied theme" + "Re-apply all brands" (the ext C52 flow). Returns each
    // brand mode's STORED recipe; a brand applied before
    ...
    // rule: the specs reply is SHARED — without the tag the edit picker's refresh
