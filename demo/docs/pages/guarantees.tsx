...
import { H2, H3, P, UL, LI, Code, Lead, Note, Table, DocLink, K } from '../prose'
...
import { CRITICAL_CLEARANCE_LC, CTA_ONFILL_ENFORCE_LC, DEFAULT_APCA_LC_MAP } from '../../../src/engine/requirements/profiles'
...
export function Body() {
  return (
    <>
      <Lead>
        Every claim the engine makes, stated exactly: which stop, against which grounds, at which bar,
      ...
      <Table
        head={['band', 'stops', 'claim', 'bar']}
        rows={[
          ['paper', <>{PAPER_0} (neutral only), {stopTokenName(1)}, {stopTokenName(2)}, {stopTokenName(3)}</>, 'grounds. Every contrast stop below is cleared against them; they carry no claim of their own', ''],
          ['highlighter', <>{stopTokenName(4)} to {stopTokenName(7)}</>, 'grounds for subtle states and decoration. The pen band is cleared against them. Never text', ''],
          [stopTokenName(8), stopTokenName(8), 'reads on every paper: focus rings, icons, borders, large text (the non-text contrast bar)', <><K v={STOP_8_NONTEXT_CONTRAST} d={1} />:1</>],
          [stopTokenName(9), stopTokenName(9), 'reads on every paper: regular text, and the emphasis fill', <><K v={PENCIL_9_CONTRAST} d={1} />:1</>],
          ['pen', <>{stopTokenName(10)}, {stopTokenName(11)}, {PEN_100} (neutral only)</>, 'reads on every paper and every highlighter, both directions', <><K v={PEN_10_CONTRAST} d={1} />:1</>],
        ]}
      />
      <P>
        ...
        text stop is a guaranteed minimum of AA. No AAA claim is made anywhere, even where a stop is placed above it.
      </P>

      <H2>What "every paper" means</H2>
      <P>
        A stop reads against the grounds of its own family and of the theme's neutral. The pen band is
      ...
      <Table
        ...
        rows={[
          [<Code>NEUTRAL_P3_WORST_SHIP_Y</Code>, <K v={NEUTRAL_P3_WORST_SHIP_Y.light} d={6} />, <K v={NEUTRAL_P3_WORST_SHIP_Y.dark} d={6} />, `the worst neutral ${stopTokenName(3)}; every chromatic family's ${stopTokenName(8)}, ${stopTokenName(9)}, ${stopTokenName(11)}`],
          ...
          [<Code>CHROMATIC_P3_WORST_SHIP_Y</Code>, <K v={CHROMATIC_P3_WORST_SHIP_Y.light} d={6} />, <K v={CHROMATIC_P3_WORST_SHIP_Y.dark} d={6} />, `the worst chromatic ${stopTokenName(3)}; the neutral's ${stopTokenName(8)} to ${stopTokenName(11)}, each at its band's bar`],
          [<Code>CHROMATIC_W80_WORST_SHIP_Y</Code>, <K v={CHROMATIC_W80_WORST_SHIP_Y.light} d={6} />, <K v={CHROMATIC_W80_WORST_SHIP_Y.dark} d={6} />, `the worst chromatic ${stopTokenName(7)}; the neutral's ${stopTokenName(10)} and ${stopTokenName(11)}`],
        ]}
      />
      <P>
      ...
      <P>
        ...
        declaration and the promise: in the shipped lane the resolver re-anchors a text stop declared against a
        paper onto {stopTokenName(3)}, the nearest paper, so clearing it clears every paper; and a declared target
        can be stricter than the claim, in which case the surplus is placement, not a promise.
      </P>
      <Table
        head={['stop', 'declared target', 'declared anchor', 'resolved against (shipped lane)', 'the promise']}
        rows={[
      ...
      <UL>
        <LI>
          <b>The pole.</b> <Code>stamp/on</Code> is white or black. The preference is perceptual: the pole with the
          larger APCA |Lc| on the fill. The law is WCAG: under the shipped lane the chosen pole must pass{' '}
          <K v={RATIO_FLOOR} d={1} />:1. If white is preferred and misses, the pole flips to black when black passes
          and reads at least Lc 45; otherwise the fill darkens until white clears 4.6:1 (the solve margin). A last
          check at the fill that ships flips the pole if the chosen one misses and the other clears.
        </LI>
        <LI>
          <b>The clearance.</b> Brand and signal fills also move, in the chosen pole's own direction, until that pole
          reads APCA Lc <K v={CO_LC} d={0} /> (the critical signal <K v={CRITICAL_CLEARANCE_LC} d={0} />). The move
          fires at the bar plus <K v={APCA_ENFORCE_MARGIN_LC} d={1} /> Lc and solves <K v={APCA_SOLVE_MARGIN_LC} d={1} /> Lc
          past it, so a value never ships on the razor. 4.5:1 stays the hard floor in every case. This is a
          legibility clearance, not a WCAG requirement.
        </LI>
        <LI>
      ...
      <UL>
        ...
        <LI>No AAA. {stopTokenName(11)} declares <K v={R11.target} d={1} />:1 against its paper, but the promise is the same AA minimum as every text stop.</LI>
        <LI>The APCA solver lane (<Code>contrastProfile: 'apca'</Code>) exists in code and is not shipped. The band claims are measured on the WCAG lane only; the APCA lane's quiet-fill pole is reported, not gated. Its target map is <K v={DEFAULT_APCA_LC_MAP[3]} d={0} /> / <K v={DEFAULT_APCA_LC_MAP[4.5]} d={0} /> / <K v={DEFAULT_APCA_LC_MAP[6.5]} d={0} /> / <K v={DEFAULT_APCA_LC_MAP[7]} d={0} /> Lc for the 3 / 4.5 / 6.5 / 7 ratios, with the on-fill bar at Lc <K v={CTA_ONFILL_ENFORCE_LC} d={0} />.</LI>
        <LI>Exact mode ships the typed hex as the stamp fill and identity with on-fill enforcement off. The label is whichever pole passes, and a note is emitted when no pole reaches Lc 60; the ramp's own stops keep their claims.</LI>
        <LI>A custom or exact secondary's ramp is the user's hex, untouched; its stops still resolve under the claims, but its stamp is not moved for the clearance.</LI>
      ...
      <Table
        ...
        rows={[
          ...
          [<Code>req:audit</Code>, '24 hues × 3 chromas at L 0.62; both modes; both solver lanes', 'every declared requirement holds on its own metric; the ladder is monotonic through the crayon; the pencil sits past the crayon; the pens order; every stop is in gamut and a valid hex; the stamp anchor respects its floor and hue; hover and pressed travel one way; the on-fill pole is valid'],
          [<Code>audit:cta-apca</Code>, 'agnostic hue × lightness × chroma grids, every stamp surface', `the clearance: every stamp fill reads Lc ${CO_LC} (critical ${CRITICAL_CLEARANCE_LC}), light and dark`],
          ...
          [<Code>audit:secondary</Code>, '5 primaries × 24 hues × 2 chromas of secondary; both lanes', 'a resolved secondary clears every effective signal or is annotated; the primary is untouched by theme resolution'],
