...
//               bumped for dark legibility; rises across the scale, peaks at the
//               highlight rung, collapses at the text stops) with an identity-
//               proportional deep-stop floor so the deep scale stops don't wash to grey.
...
//               perceptually-quiet yellow/green come up — fixing the deep scale
//               stops that "read grey"). Falls out of the H-K predictor; no per-hue constants.
//
// The cta is NOT a scale stop — darkCtaTrim (below) is unchanged, so the dark cta and
...
// Normalized per-stop dark chroma shape (highlight rung, stop 9 = 1.0), deep stops
// bumped for dark legibility. Rises across the scale, peaks at the highlight rung,
// collapses at the text stops. Deep stops 1–2 nudged up from the raw grand-mean
// (0.097/0.127) so paper-1/2 carry a touch more brand tint (they were reading slightly grey).
...
// the scale stops use the perceptual redistribution instead.
...
// Identity-proportional scale chroma FLOOR — a FRACTION of the resolved cta chroma
// (passed in as ctaC). The DEEP scale stops (paper-1/2, wash-3, stops ~1–3) wash out
// worst because their shape factor is only ~0.125–0.16; the floor lifts those. Mid+
// stops already carry enough from the shape, so the fraction tapers to 0 by ~0.50
// (the curve naturally exceeds the floor there) — and so the floor never touches the
// highlight rung (0.66) or the text stops (0.80/0.93). The fraction must EXCEED the
// deep shape (~0.125) to bite. Coefficients tuned against the dark swatch grid.
const FLOOR_FRAC = 0.22       // deep-stop floor as a fraction of ctaC (stops 1–3)
...
// trims gently on the bloom-prone hues — halfway to the loudness cap, so cool ctas
