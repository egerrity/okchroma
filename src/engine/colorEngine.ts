...
export function generateScale(
  ...
  // (darken to the WCAG-4.5 edge, search 4.6 so hex rounding never dips below
  ...
  if (opts?.highlight) {
    const isYellow =
      !hueIsNoise && Math.abs(hueDelta(brandH, YELLOW_L_LIFT.centerH)) <= YELLOW_L_LIFT.sigmaDeg
    ...
    // A highlight rung: hue + chroma from the ladder math, then (non-yellow)
    // darkened to hold white. Chroma depends on L, so enforcement re-searches
    ...
    onHighlightIsWhiteDark = !isYellow
  }
