...
// itself. Error shifts warm only (toward vermillion H~45) — cooling it
...
export interface SignalDef {
  ...
  // from them. Warning's shear is gentler than error's because yellow
  // brands have little gamut room to move within. Success and info don't
  // shear brands (info is cool by nature; green TBD).
  ...
  // check) — the ladder is deliberate: error deepest at the 0.63 default
  // (white text, severity register), info 0.70 (black text), success 0.75
  // (black text), warning naturally at 0.854.
  darkFillMinL?: number
}

export const SIGNALS: SignalDef[] = [
  // #E54D2E — vivid vermillion red
  { name: 'error',   hex: '#E54D2E', L: 0.627, C: 0.194, H: 33.3, hueShift: { cool: 0,  warm: 15 }, brandShearMaxDeg: 10, subtleChromaBoost: 1.2,  yieldChromaScale: 1 },
  // #FFC53D — bright amber (black on-fill text).
  // Yellow inverts the warm principle: the brand keeps
  // the warm gold register and WARNING flees cool toward caution-tape
  // lemon — orange-shifted warning read "too orange" and shearing the
  ...
  { name: 'warning', hex: '#FFC53D', L: 0.854, C: 0.157, H: 84.1, hueShift: { cool: 23, warm: 0 }, brandShearMaxDeg: 0, subtleChromaBoost: 1.45, yieldChromaScale: 1.15 },
  // #46A758 — bright-leaning green; dark fill brightens to 0.75 (black text)
  { name: 'success', hex: '#46A758', L: 0.651, C: 0.147, H: 147.4, hueShift: { cool: 15, warm: 10 }, brandShearMaxDeg: 0, subtleChromaBoost: 1.3,  yieldChromaScale: 1, darkFillMinL: 0.75 },
  // #6E56CF — vivid violet (closest to right as-is — no boost);
  // dark fill lifts to 0.70 so black text works
  { name: 'info',    hex: '#6E56CF', L: 0.542, C: 0.179, H: 288.0, hueShift: { cool: 15, warm: 15 }, brandShearMaxDeg: 0, subtleChromaBoost: 1.0,  yieldChromaScale: 1, darkFillMinL: 0.70 },
]
